...
Observando a relação entre os erros tipo I e II, e $t_c$: <font size=4>$H_{0}: p=0.5 \; \mbox{vs} \; H_a: p=0.7$</font>
...
## Teste de hipóteses para média ($\sigma$ desconhecido) {.build .smaller}
...
**valor crítico:** para nível de significância $\alpha=0.05$ e teste bilateral, $t_{crit}$ é tal que $P(t_{24}>t_{crit})=P(t_{24}<-t_{crit})=0.025$. De maneira que $t_{crit}=`r round(qt(.975,24),2)`$. Portanto, se $|t_{obs}|>t_{crit}$, rejeita-se $H_0$.
