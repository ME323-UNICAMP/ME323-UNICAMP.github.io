...
curve(dnorm(x, mean=1500, sd=300), 500, 2500, bty="l", yaxs = "i", xaxs="i", 
      main="Pam", ylab=expression(phi(x)), xlab="SAT", cex.lab=1.2, cex.axis=1.1, 
      cex.main=1.2, lwd=3, las=1, ylim=c(0, 0.0015))
abline(v=1800, col="red", lwd=3)

curve(dnorm(x, mean=21, sd=5), 5, 37, bty="l", yaxs = "i", xaxs="i", 
      main="Jim", ylab=expression(phi(y)), xlab="ACT", cex.lab=1.2, cex.axis=1.1, 
...
Se $Z_1,Z_2,\ldots,Z_k$ são v.a. independentes e identicamente distribuídas (i.i.d.) $N(0,1)$, temos que:

$$Q=\sum_{i=1}^kZ_i^2 \sim \chi_k^2$$

$Q$ é uma v.a. com distribuição Qui-quadrado com $k$ graus de liberdade.

Note que, se $X_1,X_2,\ldots,X_n$ são v.a. i.i.d. $N(\mu,\sigma^2)$, então:

$$Y=\sum_{i=1}^n\left(\frac{X_i-\bar{X}}{\sigma} \right)^2\sim \chi^2_{n-1}$$

## Distribuição Qui-quadrado

Uma v.a. $X$ tem distribuição qui-quadrado com $k$ graus de liberdade se sua f.d.p. é dada por:

$$f(x) = \frac{1}{2^{k/2}\Gamma(k/2)}\exp(-x/2)x^{k/2-1}\,, x\geq0$$

Note que: $X\sim\chi^2_k$ é equivalente a dizer que $X\sim Gama(k/2,1/2)$.
...
<img src="figuras/chisq_pdf.png" width=800>


## Distribuição $t$

Seja $Z \sim N(0,1)$ e $V \sim \chi_k^2$ duas v.a. independentes, então

$$T=\frac{Z}{\sqrt{V/k}}\sim t_k$$

segue um distribuição $t$ com $k$ graus de liberdade.

Obs: Se comparada com a distribuição Normal, a $t$ tem as caudas
mais pesadas.

## Distribuição $t$


Uma v.a. $X$ tem distribuição $t$ com $k$ graus de liberdade, $X\sim t_k$, se sua f.d.p. é dada por:

$$f(x)=\frac{\Gamma\left(\frac{k+1}{2}\right)}{\sqrt{k\pi}\Gamma\left(\frac{k}{2}\right)}\left(1+\frac{x^2}{k} \right)^{-\left(\frac{k+1}{2} \right)}\,,\quad x\in \mathbb{R}$$

## Distribuição $t$

<img src="figuras/t.png" width=700 style="float: left; margin-right: 20px">


## Distribuição $t$

<img src="figuras/tabelat.png" width=700 style="float: left; margin-right: 20px">


## Distribuição $t$

<img src="figuras/gosset.png" width=700 style="float: left; margin-right: 20px">

[Video - história da distribuição t](https://www.youtube.com/watch?v=hKXtrKL_nWk)


## Distribuição $t$

<img src="figuras/t_distribution_xkcd.png" width=700 style="float: left; margin-right: 20px">


## Distribuição $F$

Seja $V_1\sim\chi^2_{k_1}$ e $V_2\sim\chi^2_{k_2}$ duas v.a.'s independentes, então:

$$F=\frac{V_1/k_1}{V_2/k_2}\sim F_{k_1,k_2}$$

segue uma distribuição $F$ com $k_1$ e $k_2$ graus de liberdade.

Obs: Se $X\sim t_k$, então $X^2\sim F_{1,k}$.


## Distribuição $F$


<img src="figuras/f_pdf.png" width=700 style="float: left; margin-right: 20px">
