...
A função de distribuição acumulada $F(x)=P(X\leq x)$: 
...
$$
\begin{aligned}
\int_{-\infty}^{\infty} f(x) dx &=   \int_{-\infty}^{0}\!\! 0 dx +  \int_0^{1/2}\!\! Cx dx + \int_{1/2}^1\!\! C(1-x) dx + \int_{1}^{\infty}\!\! 0dx \\  
& = C \int_0^{1/2}\!\! x dx + C \int_{1/2}^1\!\! (1-x) dx = C \left ( \left[ \frac{x^2}{2} \right]_0^{1/2} + \left[ x- \frac{x^2}{2} \right]_{1/2}^1 \right) \\
& = C \left( \frac{1}{8} + 1 - \frac{1}{2} - \frac{1}{2} + \frac{1}{8} \right) = \frac{C}{4} \Rightarrow C \mbox{ deve ser igual a 4.} 
\end{aligned}
$$
...
## Distribuição Exponencial {.build}
...
Uma v.a. contínua $X$ segue a distribuição gama com parâmetros $\alpha$ e $\lambda$ ($\alpha>0$ e $\lambda>0$) se a f.d.p. de $X$ for dada por: 

$$f(x)=\frac{1}{\Gamma(\alpha)}x^{\alpha-1}\lambda^{\alpha}e^{-\lambda x}\,, x\geq0$$

A função gama é definida por: $\Gamma(\alpha)=\int_0^{\infty}x^{\alpha-1}e^{-x}dx$.

Para qualquer $\alpha>1$: $(\alpha-1)\Gamma(\alpha-1)$.

Para qualquer inteiro positivo $n$: $\Gamma(n)=(n-1)!$.

Caso especial: $\alpha=1$, temos $X\sim Exp(\lambda)$.

Quando $\alpha=n$, inteiro, $X$ representa o tempo de espera até que $n$ eventos ocorram.

## Distribuição Gama

<center><img src="figuras/gama_pdf.pdf" width=500></center>


## Distribuição Gama

<center><img src="figuras/gama_cdf.pdf" width=500></center>
...
Uma v.a. $X$ tem distribuição Beta com parâmetros $\alpha$ e $\beta$ ($\alpha>0$, $\beta>0$) se sua f.d.p. for dada por:
...
Obs: $X\sim Beta(\alpha=1,\beta=1)$, então $X\sim U(0,1)$.


## Distribuição Beta

<center><img src="figuras/beta_pdf1.pdf" width=500></center>


## Distribuição Beta

<center><img src="figuras/beta_pdf3.pdf" width=500></center>
...
$$E(X^k)=\frac{\Gamma(\alpha+\beta)\Gamma(\alpha+k)}{\Gamma(\alpha+\beta+k)\Gamma(\alpha)}$$
